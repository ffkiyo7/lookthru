...
交付顺序：**Discord → Telegram → 飞书**（2026-08-12 调整，原为飞书优先）。

改的理由有两条，第二条更要紧：用户手上现成有 Discord；而**飞书的 webhook 端点在境内，从 Cloudflare 出口能不能稳定到达是个未验证的问题** —— P0 那 30 小时探针测的正是这类事，并且已经部分兑现（东财行情线从 CF 出口就是不通的）。原来「大陆可直连」这条理由说的是**用户读消息**那一端，但发送是 Worker 发的，两端的网络条件不一样，当初把它们混为一谈了。

所以先用 Discord 把接口抽象磨出来。飞书适配器等到有出口证据之后再做 —— 真要做，先给它挂几天探针，别直接上线一个可能间歇性发不出去的通知渠道（发不出去的通知比没有通知更危险：人会以为「今天没事」）。

- webhook URL 是密钥，D1 中加密存储。Discord 的 URL 里直接带 token，同样适用
- 21:00 发送；部分基金净值未更新时照发，卡片内标注「N 只基金净值未更新」
- 失败重试：22:00 补发一次（必须先确认 21:00 那次是否真的成功，不能无条件补发）
- **日报与告警必须是两个独立频道 / 两个 binding**，`NotifyBinding` 带 `kind: 'DAILY' | 'ALERT'`

日报每交易日一条（约 250 条/年），告警正常情况下一条都没有。混在一个频道里，日报会把告警埋掉，而用户只有「全看」和「整个静音」两个选项 —— 这正是本项目反复遇到的那类失败。

告警频道还顺带补上了一个一直空着的口子：交易日历不可用、缺输入型 `NONE`、净值同步失败这些，**目前唯一的痕迹是 Worker 日志里的一行 `console.warn`，而没有人会去看 Worker 日志**。`/probe` 上的日历指示灯是为此补的第一块，但它要用户主动去看那一页；告警频道是第一条会主动找上门的通道。
