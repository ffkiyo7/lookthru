...
```
腾讯 / 新浪 / 东财实时分片（按近期健康度排序，最多尝试 4 个）→ 东财延时
```

- **源健康度会漂移**，因此保留隔离实例内的近期成功时间、平均延迟与 60 秒失败冷却；实例重启后回到实测初始顺序
- 单个源预算 4 秒，整条链总预算 20 秒；这条长链只供 Cron 与后台刷新使用，用户有旧行情时不会同步等待它
...
代码存档在 `apps/api/src/sources/yahoo.ts`，**未接入链路**。
...
- 激进缓存（KV + D1 / R2 last-known-good）；行情和持仓的用户请求先返回旧值、后台刷新，`null` 使用带标记的信封做负缓存，避免无效基金或缺失日历每轮重打上游/R2
