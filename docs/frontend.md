...
> **这条不变量能不能守住，取决于后端愿不愿意把旧值留着。** `/api/valuations` 目前的 KV TTL 是 60 秒，恰好等于分钟估值的刷新间隔（线上实测写入落在每分钟第 ~27 秒，余量≈0）。任何一次周期变慢或失败，接口返回的是空对象而不是一分钟前的值 —— 前端此时无论如何都只能空白，`FreshnessLine` 拿不到可标注的东西。收盘后同理：15:06 起到次日开盘前接口一直是空的。
>
> 一般规律：**陈旧度是展示层的判断，不该由存储层用过期来代劳。** 数据里已经带了 `estTime`，前端有足够信息说「14:58 的估算，行情已收盘」；把数据删掉，就把这个能力一起删了。已记入 [roadmap 的 P2 复盘](roadmap.md#p2-复盘落地当天发现的七件事)第 2、3 条。
