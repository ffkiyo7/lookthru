...
空态 / 加载态 / 陈旧态已经在 `components/states.tsx` 里备好，**不要再写一套**。`Portfolio.tsx` 与 `XRay.tsx` 顶部有标了「数据接入点」的注释块，把那几行换成 `useQuery` 就行，分支不用动；`FreshnessLine` 的 `onRetry` 记得接上 `refetch`。规则见 docs/frontend.md 不变量 #8。
...
15. **契约测试里「连不上」和「连上了不对」必须分开**。上游按地域派发源站，Actions 出口拿到的那批会间歇性 `ETIMEDOUT` / 握手后关连接 —— 这类失败 skip 并告警，**HTTP 4xx / HTML 错误页 / 非法 JSON / 断言不符必须 fail**（404 正是 fundgz 下线的签名）。全部跳过时也必须 fail，否则出口全挂那天日报是绿的。分类逻辑在 `tests/live-helpers.ts`，本身有单元测试。
   看到这个 workflow 红了，先分清是哪一种再动解析器 —— 见 docs/data-sources.md「契约测试为什么会红」。
