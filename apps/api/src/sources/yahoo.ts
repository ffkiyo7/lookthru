/**
 * 雅虎财经 —— 已评估，**不入降级链**（quotes.ts 里注释掉了）。
 *
 * 保留本模块只为存档结论，避免以后重复调研同一条路。要启用，
 * 把 quotes.ts 里那行取消注释即可，其余代码是完整可用的。
 *
 * 实测数据（2026-08-09，CF LAX 出口）：
 ...
 *         只剩 v8 chart 一次一只
 *
 * 结论：不能当主源 —— 估值引擎每分钟要刷上百只重仓股，逐只请求既违反
 * 「中央化抓取」铁律（plan 3.3），也必然被限流。而作为备源，它的独立性
 * （唯一境外源，与境内三家不相关）不足以抵消「只能小批量」这个限制，
 * 且境内已有腾讯、新浪两家实时批量源，因此搁置。
 *
