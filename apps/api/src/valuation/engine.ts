...
export function requiredQuoteSecids(input: ValuationFundInput): string[] {
  ...
  if (isPassiveIndexFund(input.fundType, input.fundName)) {
    return input.benchmark?.source === 'FUND_BENCHMARK' ? [input.benchmark.secid] : [];
  }
  return [
...
export function estimateValuation(
  ...
  if (isQdii(input.fundType, input.fundName)) {
    return result(null, null, 'NONE', 'QDII 标的市场与 A 股交易时段错开，不提供盘中估算');
  }
  if (isBondOrMoneyFund(input.fundType)) {
    return result(null, null, 'NONE', '债券型或货币型基金不提供盘中估算');
  }

  if (isExchangeTradedCode(input.fundCode)) {
    const quote = input.exchangeSecid ? quotes.get(input.exchangeSecid) : undefined;
    if (!quote) return result(null, null, 'NONE', '场内实时成交价暂不可用');
    return result(
  ...
  if (isPassiveIndexFund(input.fundType, input.fundName)) {
    if (!input.benchmark || input.benchmark.source !== 'FUND_BENCHMARK') {
      return result(null, null, 'NONE', '跟踪指数无法从业绩基准唯一确认，不猜测盘中估算');
    }
    if (prevNav === null || prevNav <= 0) {
      return result(null, null, 'NONE', '缺少前一交易日官方净值，暂不可估');
    }
    ...
    if (exposure === null) {
      return result(
        ...
        null,
        'NONE',
        linkedFund ? '业绩基准缺少目标指数权重，暂不可估' : '缺少最新股票仓位，暂不可估',
      );
    }
    const quote = quotes.get(input.benchmark.secid);
    if (!quote) return result(null, null, 'NONE', `${input.benchmark.name}行情暂不可用`);
  ...
  if (quotedWeight === 0 && residualReturn === null) {
    return result(null, null, 'NONE', '重仓股与基准行情均不可用，本轮不生成盘中估算');
  }
