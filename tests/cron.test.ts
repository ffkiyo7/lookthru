...
import { shouldRecordValuationSample } from '../apps/api/src/valuation/service';
...
describe('盘中估值引擎', () => {
  ...
  it('只在交易日 14:55 记验收样本', () => {
    ...
    expect(shouldRecordValuationSample(utc('2026-08-11T07:05:00Z'))).toBe(false);
  });
  ...
  it('空仓期也保留固定的 2 场内 + 3 指数 + 5 主动验收样本', () => {
    expect(VALUATION_CALIBRATION_CODES).toHaveLength(10);
    ...
    expect(VALUATION_CALIBRATION_CODES.slice(5)).toHaveLength(5);
  });
});
